...
const LEFT_ITEMS: NavItem[] = [
  { module: "home", icon: Home, label: "Ana" },
  ...
  const renderItem = (item: NavItem) => {
    ...
    if (item.center) {
      return (
        <button
          type="button"
          key={item.module}
          data-ocid="nav.ride.link"
          onClick={() => handleNav(item.module)}
          className="relative flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all"
          style={{
            minWidth: "52px",
            background: isActive
              ? "linear-gradient(135deg, rgba(25,230,255,0.15), rgba(181,107,255,0.1))"
              : "rgba(25,230,255,0.05)",
            border: isActive
              ? "1px solid rgba(25,230,255,0.35)"
              : "1px solid rgba(25,230,255,0.12)",
            boxShadow: isActive
              ? "0 0 16px rgba(25,230,255,0.2)"
              : "0 0 8px rgba(25,230,255,0.05)",
          }}
        >
          <item.icon
            size={20}
            style={{
              color: isActive ? "#19E6FF" : "#6B7A99",
              filter: isActive
                ? "drop-shadow(0 0 6px rgba(25,230,255,0.8))"
                : "none",
              transition: "all 0.2s",
            }}
          />
    ...
    return (
      <button
        ...
        style={{
          minWidth: "40px",
          background: isActive ? "rgba(25,230,255,0.07)" : "transparent",
          boxShadow: isActive ? "0 0 12px rgba(25,230,255,0.1)" : "none",
        }}
      >
  ...
  return (
    <nav
      ...
      style={{
        ...
        borderTop: "1px solid rgba(25,230,255,0.1)",
